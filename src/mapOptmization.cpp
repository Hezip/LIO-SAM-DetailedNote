...
//    std::string globalmap_pcd = nh.param<std::string>("globalmap_pcd", "");
    ...
    voxelgrid->setInputCloud(cloudSurfMap);
    pcl::PointCloud<PointType>::Ptr filtered(new pcl::PointCloud<PointType>());
    voxelgrid->filter(*filtered);
    cloudSurfMap = filtered;

    voxelgrid->setInputCloud(cloudCornerMap);
  ...
  */
  void allocateMemory() {
    ...
    cloudSurfMap.reset(new pcl::PointCloud<PointType>());
    cloudCornerMap.reset(new pcl::PointCloud<PointType>());
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr &pose_msg) {
    ...
    transformTobeMappedRviz[0] = roll_global;
    transformTobeMappedRviz[1] = pitch_global;
    transformTobeMappedRviz[2] = yaw_global;
    transformTobeMappedRviz[3] = x;
    transformTobeMappedRviz[4] = y;
    transformTobeMappedRviz[5] = z;
  }
  ...
  */
  void updateInitialGuess() {
    ...
    if (cloudInfo.odomAvailable == true) {
      ...
      // MARK 全局init_guess到来
      if (globalIniPoseRviz_ == true) {
        globalIniPoseRviz_ = false;

        // MARK 当前帧的姿态角（来自原始imu数据） 不需要YAW
        // MARK 加入IMU运动会不会有问题? 先不加试一试
//      Eigen::Affine3f transBack =
//          pcl::getTransformation(0, 0, 0, cloudInfo.imuRollInit, cloudInfo.imuPitchInit, 0);
//       当前帧相对于前一帧的姿态变换
...
//      Eigen::Affine3f transFinal = transTobe * transIncre;
  ...
  bool SetFilterParam(float x_l, float x_r, float y_l, float y_r, float z_l, float z_r) {

    range_cond_ = pcl::ConditionAnd<PointType>::Ptr(new pcl::ConditionAnd<PointType>());
    range_cond_->addComparison(pcl::FieldComparison<PointType>::ConstPtr(new
                                                                             pcl::FieldComparison<PointType>(
        "z",
  ...
  bool Filter(const pcl::PointCloud<PointType>::Ptr &input_cloud_ptr,
              pcl::PointCloud<PointType>::Ptr &filtered_cloud_ptr) {
    condition_filter_.setCondition(range_cond_);
    condition_filter_.setInputCloud(input_cloud_ptr);
    condition_filter_.setKeepOrganized(true);
    condition_filter_.filter(*filtered_cloud_ptr);
    return true;
  }
  ...
  void addOdomFactor() {
    if (cloudKeyPoses3D->points.empty()) {
      ...
      noiseModel::Diagonal::shared_ptr priorNoise =
          noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, M_PI * M_PI, 1e8, 1e8, 1e8)
