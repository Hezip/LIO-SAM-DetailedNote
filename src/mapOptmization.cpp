  ...
  void updateInitialGuess() {
    ...
    if (cloudInfo.odomAvailable == true) {
      ...
      if (lastImuPreTransAvailable == false) {
        ...
        // 显示增量
//              float incre_info[6];
//              pcl::getTranslationAndEulerAngles(transIncre, incre_info[3], incre_info[4], incre_info[5],
//                                                incre_info[0], incre_info[1], incre_info[2]);
//              printf("x:%.2f  y:%.2f  z%.2f\n"
//                     "roll:.2f  pitch:%.2f  yaw:%.2f\n",
//                     incre_info[3], incre_info[4], incre_info[5],
//                     incre_info[0] / M_PI * 180, incre_info[1] / M_PI * 180,incre_info[2] / M_PI * 180);
        // 赋值给前一帧
        lastImuPreTransformation = transBack;
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  */
  void updatePath(const PointTypePose &pose_in) {
    ...
    //输出数据
    odom_ofs_ << std::fixed << pose_in.time << " " << pose_stamped.pose.position.x << " "
