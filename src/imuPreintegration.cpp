...
class IMUPreintegration : public ParamServer {
  ...
  */
  IMUPreintegration() {
    // MARK 订阅rviz初始位置
    subIniPoseFromRviz = nh.subscribe("/initialpose", 8, &IMUPreintegration::initialpose_callback, this);
    ...
    // 噪声先验
    priorPoseNoise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2)
                                                             .finished()); // rad,rad,rad,m, m, m
    priorVelNoise = gtsam::noiseModel::Isotropic::Sigma(3, 1e4); // m/s
    priorBiasNoise = gtsam::noiseModel::Isotropic::Sigma(6, 1e-3); // 1e-2 ~ 1e-3 seems to be good
    // 激光里程计scan-to-map优化过程中发生退化，则选择一个较大的协方差
    correctionNoise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 0.05, 0.05, 0.05, 0.1, 0.1, 0.1)
                                                              .finished()); // rad,rad,rad,m, m, m
    correctionNoise2 =
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr &pose_msg) {
    receive_init_guess = true;
    ROS_INFO("Receive initial guess / Reset Optimizer");
  }
  ...
  */
  void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg) {
    ...
    // 0. 系统初始化，第一帧
    // MARK 收到rviz全局位置估计，重置优化器
    if (systemInitialized == false || receive_init_guess == true) {
      receive_init_guess = false;
      // 重置ISAM2优化器
      resetOptimization();
    ...
    graphFactors
        .add(gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(B(key - 1), B(key), gtsam::imuBias::ConstantBias(),
